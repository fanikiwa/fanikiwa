...
public class CommissionComponent {
    ...
    public double ComputeByCommissionRule(double Amount, CommissionRule rule)
    {
        ...
        else if (rule instanceof TieredRule)  //commpute commission in various layers/tiers
        {
            return ComputeTiered((TieredRule)rule, _amount);
        }
        else if (rule instanceof LookupRule)  //lookup a commission from a table
        {
    ...
    public double ComputeByCommissionRule(STO _sto, CommissionRule rule)
    {
        ...
        else if (rule instanceof TieredRule)  //lookup a commission from a table
        {
    ...
    private double ComputeTiered(TieredRule rule, double Amount, double min )
    {
        ...
        double commission = 0.0; double amtInBracket=0.00;
        int bracketId = 0;
        boolean cont = true;
        ...
        for (LookupRow bracket : rule.LookupTable)
        {
            ...
            if (cont)
            {
            	TieredResult res = AmountInBracket(bracket.Absolute,Amount, bracket.Min, bracket.Max, bracket.Rate, isLastBracket);
    ...
    private TieredResult AmountInBracket(boolean absolute,double TxnAmount, double From, double To, double Rate, boolean isLastBracket)
    {
    ...
    public double ComputeCommissionByTransactionType(double subject, TransactionType tt)
    {
    ...
    public CommissionRule GetCommissionRule(Long tt)
    {
        ...
        if (StringExtension.isNullOrEmpty(TT.getCommComputationMethod())) return null;
        ...
        switch (TT.getCommComputationMethod())
        {
            case "L": //tiered
                ...
                if (TT.getTieredTableId() == 0)
                    throw new  IllegalArgumentException("TieredTableId field not set in TransctionType ["+tt+"]");
    ...
    public double ComputeCommissionByTransactionType(STO _sto, TransactionType tt)
    {
    ...
    public double GetCommissionAmountForSTO(STO _sto)
    {
        ...
        {
        	STOCommSourceFlag stoflag = STOCommSourceFlag.values()[ _sto.getCommSourceFlag()];
            ...
            switch (stoflag)
            {
                ...
                case TransactionType:
                    TransactionTypeEndpoint ttep = new TransactionTypeEndpoint();
                    TransactionType DrTxnType = ttep.getTransactionType(_sto.getDrTxnType());
